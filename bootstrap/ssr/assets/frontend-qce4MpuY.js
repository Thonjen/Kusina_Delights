...
function _sfc_ssrRender$3(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  if (_ctx.$page.props.auth.user) {
    ...
    if ($data.userDropdownOpen) {
      _push(`<div class="absolute right-0.5 left-0.5 mt-2 w-45 bg-white border border-gray-200 rounded shadow-lg z-50"><a href="/dashboard" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">Dashboard</a><button class="w-full block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 text-center"> Logout </button></div>`);
    } else {
      _push(`<!---->`);
    }
    _push(`</div>`);
